...
const Index: NextPage = () => {
	...
	const filteredData = dataFinal.filter(
		(f: any) =>
			...
			f.user_nama?.toLowerCase().includes(formik.values.searchInput.toLowerCase()) &&
			f.instagram_nama?.toLowerCase().includes(formik.values.searchInput.toLowerCase())
		// // Price
					...
					<Card stretch> 
							<CardBody isScrollable className='table-responsive'>
								<table className='table table-modern table-hover'>
									...
									<tbody>
										{/** why dataFinal data have not rendering, in here???? */}
										{dataPagination(items, currentPage, perPage).map((i) => (
											<tr key={i.id}>
													...
													<div className='d-flex align-items-center'>
														<div
															...
															<div
																...
																<img
																	src={i.user_picture ?`${process.env.NEXT_PUBLIC_BASEURL}pp/${i.instagram_nama}.jpg` : INSTAGRAMLOGO}
																	alt={i.user_nama}
																	width={30}
												...
												<td>{i.instagram_nama}</td>
												{/* <td>{i.age == null ? 'n/a' : i.age}</td> */}
												<td className='text-start'>
													{i.jumlah_post}
												</td>												
												<td>{i.instagram_follower}</td>
												<td>{i.instagram_following}</td>
												<td>
													{/** random number 2500 - 12000 */} 
													{Math.floor(Math.random() * (12000 - 2500 + 1)) + 2500}													
												</td>
												<td>
												{Math.floor(Math.random() * (1000 - 100 + 1)) + 2500}													
												</td>
															...
															<DropdownItem>
																<Button
																	...
																	onClick={() => {
																		//setIdSelected(i.id)
																		//setViewDetailDataModal(true)
																		location.href = `/employee/detail/${i.id}`
																	}
																	}>
																	Lihat Detail
																</Button>
